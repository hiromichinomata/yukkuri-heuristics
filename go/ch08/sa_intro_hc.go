...
func sa(problem ProblemInput, initial ScheduleState, limit time.Duration, rng *rand.Rand) (ScheduleState, int) {
	start := time.Now()
	state := initial
...
func main() {
	...
	best, bestScore := sa(problem, initial, time.Duration(timeLimitSec*float64(time.Second)), rng)
